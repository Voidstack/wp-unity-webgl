...
class BuildExtractor {
    ...
    public function extract(): bool {
        Utils::info(__('Starting extraction', 'enosi-embedder-unity'));
        $result = true;
    ...
    private function createTempDirectory(): bool {
        if (!wp_mkdir_p($this->tmpDir)) {
            Utils::error(__('Unable to create temporary extraction folder.', 'enosi-embedder-unity'));
            return false;
        }
        return true;
    }
    ...
    private function extractZipToTemp(): bool {
        ...
        if ($zip->open($this->zipPath) !== true) {
            ...
            Utils::error(sprintf(__('Unable to open the .zip file (%s)', 'enosi-embedder-unity'), $this->zipPath));
            return false;
        }
        if (!$zip->extractTo($this->tmpDir)) {
            $zip->close();
            Utils::deleteFolder($this->tmpDir);
            Utils::error(__('Extraction failed to temporary folder.', 'enosi-embedder-unity'));
            return false;
        }
        $zip->close();
        return true;
    ...
    private function moveToTargetDirectory(): bool {
        ...
        if (!$fsSingleton->move($this->tmpDir, $this->targetDir, true)) {
            Utils::deleteFolder($this->tmpDir);
            Utils::error(__('Failed to move build to target directory.', 'enosi-embedder-unity'));
            return false;
        }
        return true;
    }
    ...
    private function verifyExtractedFiles(): bool {
        ...
        Utils::info(__('Files found: ', 'enosi-embedder-unity') . Utils::arrayToString($foundFiles));
